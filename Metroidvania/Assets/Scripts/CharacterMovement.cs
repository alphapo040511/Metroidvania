...
public class CharacterMovement : MonoBehaviour
{
    ...
    [SerializeField] private LayerMask m_WahtIsGround;                                      //캐릭터에게 땅이 무것인지 결정하는 마스크
    ...
    private bool m_FacingRight = true;                      //플레이어가 현재어느 방향을 바라보고 있는지 결정
    ...
    private float jumpWallDisX = 0;                         //플레이어와 벽 사이의 거리
    private bool limitVeloOnWallJump = false;               //낮은 FPS에서 벽 점프 거리를 제한하기 위한 변수
    ...
    private void Awake()                                    //컴포넌트들을 가져와서 세팅한다.
    {
        ...
        if (OnFallEvent == null)                            //이벤트가 추가 안되어 있을 떄 이벤트를 할당한다.
    ...
    private void FixedUpdate()              //FixedUpdate에서 이동 구현을 한다.
    {
        bool wasGrounded = m_IsGrounded;    //땅 체크 값을 이전 변수에 넣어서 매 프레임마다 갱신
        m_IsGrounded = false;

        //GroundCheck 위치에서 땅으로 지정된 것과 겹치는지 확인
        Collider2D[] groundColliers = Physics2D.OverlapCircleAll(m_GroundCheck.position, k_GroundedRadius, m_WahtIsGround);

        for(int i = 0; i< groundColliers.Length; i++)
        {
            if(groundColliers[i].gameObject != gameObject)
            {
                m_IsGrounded = true;
                if(!wasGrounded)
                {
                    ...
                    if (m_Rigidbody2D.velocity.y < 0f) limitVeloOnWallJump = false;
                }
            }
        }

        if (!m_IsGrounded) OnFallEvent.Invoke();            //땅이 아닐때 떨어지고 있다는 이벤트를 호출
        ...
        Collider2D[] wallColliers = Physics2D.OverlapCircleAll(m_WallCheck.position, k_GroundedRadius + 0.3f , m_WahtIsGround);
        for (int i = 0; i < wallColliers.Length; i++)
        {
            if(wallColliers[i].gameObject != gameObject)
            {
                m_IsWall = true;
                if(!m_IsGrounded && !status.isDashing && m_Rigidbody2D.velocity.y < 0)
                {
                    m_IsWallSlidding = true;
                    if(!wasWallSlidding)            //이전 프레임이 벽이아니였다면 체크하여 돌려서 벽을 슬라이딩 하고 검출 하게 한다.
                    {
        ...
        if(limitVeloOnWallJump)
        {
            if (m_Rigidbody2D.velocity.y < -0.5f) limitVeloOnWallJump = false;
            ...
            else if(jumpWallDisX < -2f)
            {
                limitVeloOnWallJump = false;
    ...
    public void Move(float move, bool jump, bool dash)
    {
        if(status.canMove)      //클래스에서 이동이 가능하다고 하면
        {
            ...
            else if(m_IsGrounded || m_AirControl)       //플레이어가 땅에 닿았거나 공중 제어가 켜져있을때
            {
                ...
                {
                    Vector3 targetVelovity = new Vector2(move * 10.0f, m_Rigidbody2D.velocity.y);       //목표 속도를 계산
                    m_Rigidbody2D.velocity = Vector3.SmoothDamp(m_Rigidbody2D.velocity, targetVelovity, ref velocity, m_MovementSmoothing);     //속도를 부드럽게 변경

                    if(move > 0 && !m_FacingRight && !m_IsWallSlidding)         //입력이 오른쪽, 플레이어가 왼쪽을 보고있다면
                    {
                        Flip();                                                 //플레이어를 뒤집는다.
                    }
                ...
                {
                    animator.SetBool("IsJumping", true);                    //애니메이터 점프 상태 전달
