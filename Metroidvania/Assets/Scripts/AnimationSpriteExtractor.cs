...
public class AnimationSpriteExtractor : EditorWindow
{
    ...
    private List<SpriteInfo> spriteInfosList = new List<SpriteInfo>();      //스프라이트 정보를 저장할 리스트

    [MenuItem("Window/Animation Sprite Extractior")]     //메뉴에 Animation Sprite Extractior 항목을 추가
    public static void ShowWindow()
    {
        GetWindow<AnimationSpriteExtractor>("Animation Sprite Extractior");     //에디터 창 열기
    }

    private void OnGUI()
    {
        GUILayout.Label("Extract Sprites Info form Animation Clip", EditorStyles.boldLabel);        //에이터 창에 레이블과 애니메이션 클립 필드 표시
        ...
        {
            if(GUILayout.Button("Extract Sprite Info"))     //버튼이 클릭 되면 스프라이트 정보를 추출
            {
                ExtractSpritesInfo(animationClip);
            }

            if(spriteInfosList.Count > 0)       //스프라이트 정보 리스트가 비어있지 않은 경우, 리스트의 내용을 표시
            {
    ...
    {
        spriteInfosList.Clear();        //기존 스프라이트 정보 초기화
        ...
        foreach (var binding in bindings)    //각 바인딩을 순회
        {
            if (binding.propertyName.Contains("Sprite")) //바인딩된 프로퍼티가 스프라이트일 경우
            {
                ...
                foreach (var keyframe in keyframes)      //각 키프레임을 순회
                {
                    ...
                    if (sprite != null)
                    {
                        spriteInfosList.Add(new SpriteInfo { time = keyframe.time, spriteName = sprite.name }); //스프라이트 정보를 리스트에 추가
                    }
                }
            }
        }
