...
public class Enemy : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(knockbackCounter > 0)                        //넉백 시간 체크
        {
            ...
            if(knockbackCounter <= 0)                   //넉백 완료 되면
            {
                rigidbody.velocity = Vector2.zero;      //시간 초기화
            }
        }
    ...
    {
        if(collision.gameObject.CompareTag("Player") && life > 0 && isInvincible)                           //CompareTag 는 == 와 같은 의미로 쓰임
        {
            collision.gameObject.GetComponent<CharacterStatus>().ApplyDamage(2, transform.position);
        }
    }
    
    void EndInvincible()                //무적이 끝났을 때 상태값을 초기화 시킨다.
    {
        isHitted = false;
        isInvincible = false;
    ...
    void StartDestroySequence()         //적군 피격후 설정값 함수
    {
